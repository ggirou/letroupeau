    ...
    public void drawShadow() {
        ...
        GL11.glPushAttrib(GL11.GL_CURRENT_BIT | GL11.GL_COLOR_BUFFER_BIT | GL11.GL_TEXTURE_BIT | GL11.GL_LIGHTING_BIT | GL11.GL_DEPTH_BUFFER_BIT);
        {
        	...
        	GL11.glPushMatrix();
        	{
        		...
        		GLApp.clearMask();      // clear the stencil buffer
        		GLApp.beginMask(1);     // we'll fill the shadow shape with the number 1
        		drawObject();           // draw the object into stencil buffer 
        		GLApp.endMask();        // done with mask
        		GLApp.activateMask(1);  // now OpenGL will draw only where stencil buffer == 1
