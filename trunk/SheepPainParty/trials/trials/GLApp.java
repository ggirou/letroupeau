...
public class GLApp {
    ...
    // initDisplay() will pick a Display that best matches
    // the values below, and will set viewport dimensions
    // to match the aspectRatio value above.
    ...
    public static IntBuffer     bufferSettingInt = allocInts(16);
    ...
    public static ByteBuffer    bufferZdepth = allocBytes(SIZE_FLOAT);      // for getZDepth()
    public static ByteBuffer 	bufferStencilValue = allocBytes(SIZE_BYTE);   // for getStencilValue()
    public static FloatBuffer   tmpResultProject = allocFloats(16);
    public static FloatBuffer   tmpResultUnProject = allocFloats(16);
    public static FloatBuffer   tmpLightPos = allocFloats(4);    // temp var to hold light position
    ...
    public static final float PIOVER180 = 0.0174532925f;   // A constant used in navigation
    ...
    public static boolean initDisplay() {
        ...
        // Set display mode
        try {
            if (useCurrentDisplay) {  // use current display settings (ignore properties file)
                DM = origDM;
     ...
     * once when app starts, but may be called again restore settings to a
    ...
    /**
     *  Placeholder function.  Subclass will override this.  Used by GLGiantScreenshot.java.
     */
     ...
     * @param configFilename
     */
    public Properties loadPropertiesFile(String configFilename)
    {
    ...
    public int getPropertyInt(Properties props, String propName) {
    ...
    public float getPropertyFloat(Properties props, String propName) {
        String s = getProperty(props,propName);
        float v = -1f;
        if (s != null) {
    ...
    public boolean getPropertyBool(Properties props, String propName) {
        String s = getProperty(props,propName);
        boolean v = false;
        if (s != null) {
    ...
    public static void project(float x, float y, float z, float[] resultf)
    {
        ...
        GLU.gluProject( x, y, z, getModelviewMatrix(), getProjectionMatrix(), getViewport(), tmpResultProject);
        resultf[0] = tmpResultProject.get(0);
        resultf[1] = tmpResultProject.get(1);
        resultf[2] = tmpResultProject.get(2);
    }
    ...
    {
        GLU.gluUnProject( x, y, z, getModelviewMatrix(), getProjectionMatrix(), getViewport(), tmpResultUnProject);
        resultf[0] = tmpResultUnProject.get(0);
        resultf[1] = tmpResultUnProject.get(1);
        resultf[2] = tmpResultUnProject.get(2);
    }
    ...
    /**
     * Create a texture from the given pixels in the default OpenGL RGBA pixel format.<BR>
     * Configure the texture to repeat in both directions and use LINEAR for magnification.
     * <P>
     * @return  the texture handle
     */
    ...
    /**
     * Set the screen area in which the OpenGL context will draw. Also
     * set the aspect ratio (width / height) for use in the setPerspective() function.
     * 
     * @param x
     * @param y
     * @param width
   ...
   /**
    ...
    {
    	tmpLightPos.put(new float[] {x,y,z,1}).flip();
        GL11.glLight(GLLightHandle, GL11.GL_POSITION, tmpLightPos);
    }
    ...
    {
        tmpLightPos.put(position).flip();
        GL11.glLight(GLLightHandle, GL11.GL_POSITION, tmpLightPos);
    }

    /**
     * disable the given light 
     */
    public static void setLightOff(int GLLightHandle)
    {
	...
	*/
    ...
    /**
     * Draw an image in ortho mode (2D) over the entire screen area.
     * Convert the image to a texture and map onto screen-sized quad.  
    ...
    public static void drawImageFullScreen(GLImage img) {
    	...
    	// Calculate the UV position of the image ? 
    	...
    	GL11.glBegin(GL11.GL_QUADS);
    	{
    		...
    		GL11.glVertex3f(displayMode.getWidth(), 0f, 0f);         // Bottom Right
    		
    		GL11.glTexCoord2f(maxU, maxV);
    		GL11.glVertex3f(displayMode.getWidth(), displayMode.getHeight(), 0f);   // Top Right
    		
    		GL11.glTexCoord2f(0, maxV);
    		GL11.glVertex3f(0f, displayMode.getHeight(), 0f);       // Top left
    	}
    	GL11.glEnd();
     ...
     */
    public static ByteBuffer getFramePixels(int x, int y, int w, int h) {
    ...
    {
        frameSave(txtrHandle, 0,0, DM.getWidth(),DM.getHeight());   // entire screen
    }
     ...
     * @see makeTextureForScreen()
     */
    public static void frameSave(int txtrHandle, int x, int y, int w, int h)
    {
     ...
     */
    public void renderCube(float size, int segments) {
    ...
    /**
      ...
      * by buildFont().
      */
     public static void print(int x, int y, int set, String msg)
     {
	...
	public static void savePixelsToPNG(ByteBuffer framebytes, int width, int height, String imageFilename, boolean flipY) {
      ...
      */
     public static int getStencilValue(int x, int y)
     {
    	 bufferStencilValue.clear();
    	 // read the stencil value at the given position, as an unsigned byte
    	 GL11.glReadPixels(x, y, 1, 1, GL11.GL_STENCIL_INDEX, GL11.GL_UNSIGNED_BYTE, bufferStencilValue);
    	 return (int) bufferStencilValue.get(0);
     }
      ...
      * @see endDisplayList()
      */
    public void callDisplayList(int displayListID) {
	 ...
	 */
     ...
     public static InputStream getInputStream(String filename) {
    	 ...
    	 try {
    		 ...
    		 err("GLApp.getInputStream (" + filename + "): " + ioe);
    		 if (in != null) {
    			 try {
    				 in.close();
    			 }
    		 ...
    		 err("GLApp.getInputStream (" + filename + "): " + e);
    	 }
